...
import "@openzeppelin/contracts/utils/Counters.sol"; // if you see "Expected string literal (path), "*" or alias list." error just ignore it 
import "hardhat/console.sol"; // the same at the upper line
...
contract Create {
    ...
    struct Candidate {
        ...
        uint256 voteCount;
        address _address;
        string ipfs; // Contain all the information of the candidate
    }
    ...
    function voterRight(address _address, string memory _name, string memory _image, string memory _ipfs) public {
        require(votingOrganizer == msg.sender, "Only organizer can be create voter");
